...
class AjaxController extends BaseController
{
    ...
    public function ajax(Request $request, DataExchange $dataExchangeService)
    {
        ...
        $galleryKeyCache = $this->formatCacheKey(Constants::CACHE_PHOTOS_GALLERY_IMAGES_DATA, $galleryId);
        ...
        if ($galleryExchange === false) {
            ...
            if ($data) {
                $galleryExchange = $dataExchangeService->ExchangeAllDetailGalleryPhotoData($data, Constants::MOBILE_IMAGE_THUMB_POPUP, Constants::MOBILE_IMAGE_HOME_TOPIC);
            } else {
                $galleryExchange = [];
            }
    ...
    public function list(Request $request, DataExchange $dataExchangeService, CryptUtils $cryptUtils)
    {
        ...
        $photosKeyCache = $this->formatCacheKey(Constants::CACHE_PHOTOS_PAGE, $pages);
        ...
        if ($photoGalleries === false) {
            $em = $this->getDoctrine()->getManager();
            $data = $em->getRepository(PhotoGalleries::class)->getPhotoGalleriesPaging($pages, Constants::PAGE_SIZE_MOBILE + 1);
            if ($data) {
        ...
        if ($photoGalleriesCount > Constants::PAGE_SIZE_MOBILE) {
