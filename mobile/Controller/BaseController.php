...
{
    const LIMIT_ITEMS = 48;
    /**
     * Get cache provider for all controller
     * author: ThanhDT
     * date:   2018-12-17 08:51 AM
     * @param $serverCache
    ...
    {
        $key_author = $this->formatCacheKey(Constants::TABLE_AUTHOR_USERNICENAME, $userSlug);
    ...
    {
        $key_author = $this->formatCacheKey(Constants::TABLE_AUTHOR_USERID, $userId);
        $service_cache = $this->getCacheProvider(Constants::SERVER_CACHE_ARTICLE);
        $author = $service_cache->get($key_author);
        if ($author === false) {
            $em = $this->getDoctrine()->getManager();
    ...
    {
        $service_cache = $this->getCacheProvider(Constants::SERVER_CACHE_ARTICLE);
        $keyDetail = $this->formatCacheKey(Constants::TABLE_ARTICLE_DETAIL_URL_BY_SLUG, $slug);
    ...
    {
        $service_cache = $this->getCacheProvider(Constants::SERVER_CACHE_ARTICLE);
        $keyDetail = $this->formatCacheKey(Constants::TABLE_ARTICLE_DETAIL_SHORT_BY_SLUG, $slug);
    ...
    {
        if ($args) {
            return Constants::MOBILE_AMP . vsprintf($format, $args);
        }

        return Constants::MOBILE_AMP . $format;
    }
