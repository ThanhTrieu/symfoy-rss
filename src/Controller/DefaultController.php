    ...
    public function index(DataExchange $exchangeService, Category $category, Request $request, CryptUtils $cryptUtils)
    {
        ...
        if (($focusPosts = $cacheService->get($focusCacheKey)) === false) {
            ...
            if ($sourcePosts) {
                ...
                $focusTopCenter = $exchangeService->exchangeArrayArticle($focusTopCenter, Constants::POST_AVATAR_FOCUS_TOP1_SIZE);
                $focusTopLeft = $exchangeService->exchangeArrayArticle($focusTopLeft, Constants::POST_AVATAR_FOCUS_TOP2_SIZE);
                $focusTopRight = $exchangeService->exchangeArrayArticle($focusTopRight, Constants::POST_AVATAR_FOCUS_TOP2_SIZE);
                $focusPosts = [
